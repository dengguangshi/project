...
class User extends Base
{
  ...
  public function index()
  {
    ...
    $user = new UserModel();
    $lists['data'] = $user->getUsersByWhere($map, $Nowpage, $limits);
    $lists['count'] = Db::name('admin')->where($map)->count();
    foreach ($lists['data'] as $k => $v) {
      $lists['data'][$k]['last_login_time'] = date('Y-m-d H:i:s', $v['last_login_time']);
    }
...
//    $this->assign('list',$lists);
  ...
  public function state()
  {
    ...
    if ($status == 1) {
      ...
      return json(['code' => 200, 'data' => $flag['data'], 'msg' => '已开启']);
    }
  }
